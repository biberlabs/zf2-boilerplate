...
## Database Setup
 1. Create a database named `boilerplate`. Thanks to Doctrine 2, you can use MySQL, PostgreSQL or SQLite.
 2. Create a local copy of doctrine.local.php.dist file using the commands below:
...
```
 ...
 3. By default, ZF2 boilerplate ships with an example PDO PostgreSQL configuration in `doctrine.local.php`, if you prefer switch to MySQL, just replace the `driverClass` to :
   - `Doctrine\DBAL\Driver\PDOPgSql\Driver` for PostgreSQL 
   - `Doctrine\DBAL\Driver\PDOMySql\Driver` for MySQL
   - `Doctrine\DBAL\Driver\PDOSqlite\Driver` for SQLite
 4. Replace the `@dbuser`, `@dbpass` and `@dbname` in `doctrine.local.php` file with yours.
 5. Switch to command line back and create required tables with the help of schema-tool:

```
$ php public/index.php orm:schema-tool:create
```

 6. And populate them with some sample data using data fixtures:
